...
export default function BookingPage() {
    ...
    useEffect(() => {
        if (step === 3 && dateInputRef.current) {
            setTimeout(() => {
                try { dateInputRef.current?.showPicker(); } catch (e) { console.log('showPicker not supported', e); }
            }, 300);
        }
    }, [step]);
        ...
        try {
            ...
            if (res.data.success) {
                ...
                if (deviceId && finalPhone) {
                    try { await api.post('/appointments/customers/sync', { device_id: deviceId, customer_phone: finalPhone, push_token: localStorage.getItem('push_token') }); } catch (syncErr) { }
                }
                localStorage.removeItem('pending_booking');
                navigate('/my-appointments', { replace: true });
            }
        } catch (err: any) { alert('Hata: ' + (err.response?.data?.error || err.message)); }
    };
                                ...
                                {services.map(s => {
                                    ...
                                    return (
                                        <button key={s.id} onClick={() => {
                                            const newIds = isSelected ? selection.serviceIds.filter(id => id !== s.id) : [...selection.serviceIds, s.id!];
